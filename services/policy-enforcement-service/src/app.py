...
class App(FastAPI):
    def openapi(self) -> Dict[str, Any]:
        ...
        for target in policy_checker.services:
            ...
            scheme_builder.append(
                resp.json(), inject_token_in_swagger=target.inject_token_in_swagger
            )
...
async def catch_all(request: Request, path_name: str):
    ...
    return StreamingResponse(
        rp_resp.aiter_raw(), status_code=rp_resp.status_code, headers=rp_resp.headers
    )
